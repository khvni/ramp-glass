...
| ID | Task | Size | Depends on | Status | Notes |
|----|------|------|------------|--------|-------|
| 6.1 | Add `app_settings` SQLite table (key/value JSON) in `@tinker/memory/database.ts`. CRUD helpers in new `packages/memory/src/settings-store.ts`. | S | — | in progress | [khvni/tin-57-app-settings · ETA 2026-04-21] Shared config surface. |
