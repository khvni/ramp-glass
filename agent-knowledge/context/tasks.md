...
| 6.12 | Seed Memory demo scaffold from Paper IQ-0 into the actual empty-memory bootstrap: shared seed notes, real sidebar counts, Paper article default selection, and browser preview parity without `VITE_E2E`-only data. | M | 6.3, 6.10 | review | TIN-207 · PR #124. Shared seed scaffold now drives bootstrap + browser preview + visual baseline. |
